...
namespace SerialPortListener.Serial
{
    ...
    public class SerialPortManager : IDisposable
    {
        ...
        public void StartListening()
        {
            try
            {
                ...
                if (_serialPort != null && _serialPort.IsOpen)
                    _serialPort.Close();
                string com_port_name_read = "COM1";
                if(!File.Exists("port.ini"))
                {
                    MessageBox.Show("Error: File 'port.ini' doesn't exist.");
                    return;
                }
                string line;
                // Read the file and display it line by line.
                System.IO.StreamReader file =
                   new System.IO.StreamReader("port.ini");
                if ((line = file.ReadLine()) != null)
                {
                    com_port_name_read = line;
                }
                ...
                _serialPort = new SerialPort(
                    _currentSerialSettings.PortName = com_port_name_read,
        ...
        public void SerialSend(string tosend)
        {
